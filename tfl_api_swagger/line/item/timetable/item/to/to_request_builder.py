...
class ToRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /Line/{ids-id}/Timetable/{fromStopPointId}/to
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/Line/{ids%2Did}/Timetable/{fromStopPointId}/to", path_parameters)
    
    def by_to_stop_point_id(self,to_stop_point_id: str) -> WithToStopPointItemRequestBuilder:
        """
        Gets an item from the tfl_api.Line.item.Timetable.item.to.item collection
