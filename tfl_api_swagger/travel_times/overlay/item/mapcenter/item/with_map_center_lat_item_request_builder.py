...
class WithMapCenterLatItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /TravelTimes/overlay/{z}/mapcenter/{mapCenterLat}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/TravelTimes/overlay/{z}/mapcenter/{mapCenterLat}", path_parameters)
    
    def by_map_center_lon(self,map_center_lon: float) -> WithMapCenterLonItemRequestBuilder:
        """
        Gets an item from the tfl_api.TravelTimes.overlay.item.mapcenter.item.item collection
