...
class PinlocationRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /TravelTimes/overlay/{z}/mapcenter/{mapCenterLat}/{mapCenterLon}/pinlocation
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/TravelTimes/overlay/{z}/mapcenter/{mapCenterLat}/{mapCenterLon}/pinlocation", path_parameters)
    
    def by_pin_lat(self,pin_lat: float) -> WithPinLatItemRequestBuilder:
        """
        Gets an item from the tfl_api.TravelTimes.overlay.item.mapcenter.item.item.pinlocation.item collection
