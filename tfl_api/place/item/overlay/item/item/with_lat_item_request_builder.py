...
class WithLatItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /Place/{-id}/overlay/{z}/{Lat}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/Place/{%2Did}/overlay/{z}/{Lat}", path_parameters)
    
    def by_lon(self,lon: str) -> WithLonItemRequestBuilder:
        """
        Gets an item from the tfl_api.Place.item.overlay.item.item.item collection
