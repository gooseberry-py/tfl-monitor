...
class WithWidthItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /Place/{-id}/overlay/{z}/{Lat}/{Lon}/{width}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/Place/{%2Did}/overlay/{z}/{Lat}/{Lon}/{width}", path_parameters)
    
    def by_height(self,height: int) -> WithHeightItemRequestBuilder:
        """
        Gets an item from the tfl_api.Place.item.overlay.item.item.item.item.item collection
